...
public class TwitterPageRankAlgorithm {
    ...
    public void addWeighttoAllNodes() {
    	...
    	for (Node node : graph.getNodes()) {
    		String index = node.getLabel();
    		//System.out.println("Đang check index = " + index);
    		if(index.startsWith("user_")) {
    			//System.out.println("Đang check index = " + index);
    			User user = manager.getUserById(index);
    			if(user != null) {
    			 double numFollowers = (double)user.getFollowersCount();
    			 
       		     Math.log(numFollowers);
       		     node.setAttribute("weight", 10.0);
       		    // System.out.println("Weight của user " + index + " = " + weight );
       		     if(!user.getTweets().isEmpty()) {
       		    	 for(Tweet tweet : user.getTweets()) {
       		    		 String tweetIndex = tweet.getId();
       		    		 String likeCount = tweet.getLikeCount();
       		    		 System.out.println("Đang xử lý "+ tweetIndex);
       		    		 int numLikeCount = TextToIntegerConverter.convertTextToInteger(likeCount);
       		    		 String repostCount = tweet.getRepostCount();
       		    		 int numRepostCount = TextToIntegerConverter.convertTextToInteger(repostCount);
       		    		 int sum = numLikeCount + numRepostCount;
       		    		 double tweetWeight = 0.5;
       		    		 if(sum!=0) tweetWeight  = Math.log((double) sum);
       		    		
       		    		 Node tweetNode = graph.getNode(tweetIndex);
       		    		 tweetNode.setAttribute("weight", tweetWeight);
       		    		System.out.println("Tổng Weight của tweet " + tweetIndex + " của " + index +  " = " + tweetWeight );
       		    		 System.out.println("Weight của tweet " + tweetIndex + " của " + index +  " = " + tweetWeight );
       		    	 }
       		     }
    			}else {
    				node.setAttribute("weight", 1.0);
    			}

    		}
        }
    	for (Node node : graph.getNodes()) {
    		sumWeight = sumWeight + (double) node.getAttribute("weight");	
    	}
    	for (Node node : graph.getNodes()) {
    		double currentWegiht = (double)node.getAttribute("weight");	
    		double updatedWeight = currentWegiht/sumWeight;
    		node.setAttribute("weight", updatedWeight);
    	}
    	try {
			Thread.sleep(6000);
